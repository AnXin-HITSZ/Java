...
**1. （北京明 \*\* 信）父类哪些成员可以被继承？属性可以被继承吗？可以或者不可以，请举下例子。**
答：
* 父类的属性、方法可以被继承。
* 父类的构造器可以被子类调用。
...
**1. （顺 \*、软 \*\* 力、明 \* 数据、阳 \* 科技、中 \* 软）什么是 Override？与 Overload 的区别？**
答：
* 略。

**2. （新 \* 陆）Overload 的方法是否可以改变返回值的类型？**
答：
* 重载的方法只和方法的方法名和参数列表有关，与返回值的类型无关。

**3. （鸿 \* 网络、深圳德 \*\* 技、航 \*\* 普）构造器 Constructor 是否可被 override？**
答：
* 不能！
* 构造器可以重载。

**4. （腾 \*）为什么要有重载？我随便命名一个别的函数名不行吗？谈谈你是怎么理解的。**
答：
* 见名知意。

### 9.3 `super` 关键字

**1. （蚂 \*\* 服）`super` 和 `this` 的区别。**
答：
* 把两个关键字各自的特点说清楚即可。

**2. （北京楚 \* 龙）`this`、`super` 关键字分别代表什么？以及它们各自的使用场景和作用。**
答：
* 略。

### 9.4 多态

**1. （三 \* 重工、江 \* 智能、银 \* 数据、君 \* 科技）谈谈你对多态的理解。**
> 类似问法：
> * （国 \* 电网）Java 中实现多态的机制是什么？
> * （上 \* 银行）什么是多态？
> * （贝 \*）Java 中的多态是什么意思？
...
**2. （万 \* 智能）多态 `new` 出来的对象跟不多态 `new` 出来的对象区别在哪？**
答：
*  多态 `new` 出来的对象体现了虚方法调用，屏蔽了子类特有的属性和方法。例如：
```java
Person p = new Man();   // 虚方法调用；屏蔽了子类 Man 类特有的属性和方法
```

**3. （楚 \* 龙）说说你认为多态在代码中的体现。**
答：
* 无处不在！（略）
...
**1. （拓 \* 思）`==` 与 `equals` 的区别？**
> 类似问法：
> * （华油 \*\* 普）两个对象 `A` 和 `B`，`A == B`、`A.equals(B)` 有什么区别？
答：
* 略。

**2. （安 \*\* 网络科技）重写 `equals()` 方法要注意什么？**
答：
*  明确判定两个对象实体 `equals()` 方法的标准，是否需要所有的属性参与。
*  对象的属性，且是自定义的类型，则此属性也需要重写 `equals()` 方法。

**3. （阿 \* 校招）Java 中所有类的父类是什么？它都有什么方法？**
> 相关问题：
> * （恒 \* 电子）`Object` 类有哪些方法？
